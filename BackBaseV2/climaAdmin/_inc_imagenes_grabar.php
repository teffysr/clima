...
if (cargar('idImagen') && cargar('eliminarImagen')) {
	if ($row = $db->fetchOneRow("SELECT imagen, extension FROM ".TABLA_IMAGENES." WHERE id = '{$idImagen}'")){
		if ($BORRADO_FISICO) {
	...
	borrarRegistro(TABLA_IMAGENES, $idImagen);	
	exit("<script>history.back();</script>");
}

if (cargar('idImagen') && cargar('activarImagen')) {
	activarRegistro(TABLA_IMAGENES, $idImagen);
	exit("<script>history.back();</script>");
}

if (cargar('idImagen') && cargar('principalImagen')) {
	principalRegistro(TABLA_IMAGENES, $idImagen, TABLA_IMAGENES_FK, $id);
	exit("<script>history.back();</script>");
}
