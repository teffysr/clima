	...
	if($('.form-validate').length > 0)
	{
		$('.form-validate').each(function(){
			...
			$("#"+id).validate({
				...
				/*,
				success: function(label) {
					$(label).closest('.control-group').removeClass('error success').addClass('valid').addClass('success');
				}
				*/
			});
		});
...
function applyTableStyles() {
	...
	if($('.dataTable').length > 0){
		$('.dataTable').each(function(){
			...
			$(this).dataTable(opt);
			$('.dataTables_filter input').attr("placeholder", "Buscar aqui...");
		});
	}

	/*
	Oculta los botones de ordenamiento en caso de que se haga click sobre otra columna
	Muestra los botones de ordenamiento y ordena la tabla por el orden final
	*/
	$('.dataTable th').on('click', function(){
		$('td.ordenPryBotonesTD > *').hide();
	})
	$('.dataTable th.ordenPryBotonesTH').on('click', function(){
		$('th.ordenPryTH').click();
		$('td.ordenPryBotonesTD > *').show();
		return false;
	});
	$('.dataTable th.ordenPryTH, .dataTable td.ordenPryTD').hide();
}